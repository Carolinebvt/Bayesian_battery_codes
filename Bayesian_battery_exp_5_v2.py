...
from Bayesian_battery_functions_v2 import random_pictures, rate_confidence, \
     create_note, play_notes, select_response, associate_morph_tone, define_tones
...
def execute_trial_5(block_nb, shuffle = True, param_1 = False, param_2 = False):
    ...
    window is presented for the 4th morph. 
...
#Load morphes for block2 with 40%H(50%, noise 4 and 5)
...
#Load morphes for block3 with 20%H(50%, noise 4 and 5)
...
#Load morphes for block4 with 50%H(50%, noise 4 and 5)
...
exp.clock.wait(time_prior)
play_notes(A_low, A_80_low, A_high, A_80_high)
execute_trial_5(block2, shuffle = True)    
exp.clock.wait(time_interblocks)
...
exp.clock.wait(time_prior)
play_notes(A_low, A_80_low, A_high, A_80_high)
execute_trial_5(block3, shuffle = True)
exp.clock.wait(time_interblocks)
...
exp.clock.wait(time_prior)
play_notes(A_low, A_80_low, A_high, A_80_high)
execute_trial_5(block4, shuffle = True)
exp.clock.wait(time_interblocks)
...
##to see data : 
#expyriment.misc.data_preprocessing.read_datafile(path+'/Bayesian_battery_codes/Bayesian_battery_exp_5.2_08_201909191242.xpd')    
